...
const ESPS = {
  ...
  "ORL":             { icon:"👂", c:"#3B6D11", terms:["otorrinolaringología","otorrinolaringologia","otorrino","orl","audiometría","audiometria","impedanciometría","impedanciometria","audífono","audifono","hipoacusia"] },
  ...
  "Laboratorio":     { icon:"🧪", c:"#888780", terms:["laboratorio clínico","laboratorio clinico","exámenes de laboratorio","hemograma","bioquímica","bioquimica","microbiología","microbiologia","anatomía patológica","anatomia patologica","prestaciones de laboratorio","convenio de suministro de exámenes externos de laboratorio"] },
};
...
const SNAPSHOT_RAW = [
  ...
  {id:"1057415-24-LE26",nombre:"SERVICIO ODONTOLOGICO DE REHABILITACION CON PRÓTESIS",cod:5,cierre:"2026-06-11T15:00:00",org:"Hospital de Los Ángeles",region:"Región del Biobío"},
  ...
  {id:"1057415-30-LE26",nombre:"SERVICIO ODONTOLOGICO INTEGRAL",cod:5,cierre:"2026-06-11T17:00:00",org:"Hospital de Los Ángeles",region:"Región del Biobío"},
...
export default function App() {
  ...
  const [lastFetch,  setLastFetch]  = useState("04/06/2026");
  ...
  const actualizarDesdeAPI = async () => {
    ...
    try {
      ...
      if(!listado) try{ const r=await fetch(`https://api.allorigins.win/raw?url=${encodeURIComponent(url)}`); const d=await r.json(); if(d?.Listado) listado=d.Listado; }catch{}
      if(!listado) try{ const r=await fetch(`https://corsproxy.io/?url=${encodeURIComponent(url)}`); const d=await r.json(); if(d?.Listado) listado=d.Listado; }catch{}
      if (listado) {
  ...
  const fetchDetalle = async id => {
    ...
    const url=`https://api.mercadopublico.cl/servicios/v1/publico/licitaciones.json?codigo=${id}&ticket=${ticket}`;
    let item=null;
    try{ const r=await fetch(url); const d=await r.json(); item=d?.Listado?.[0]; }catch{}
    if(!item) try{ const r=await fetch(`https://api.allorigins.win/raw?url=${encodeURIComponent(url)}`); const d=await r.json(); item=d?.Listado?.[0]; }catch{}
